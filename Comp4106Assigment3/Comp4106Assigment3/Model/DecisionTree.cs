    ...
    public class DecisionTree
    {
        ...
        public Node CreateTree(IEnumerable<double[]> vectors, int numberOfClasses, double entropy, int depth)
        {
            if(entropy == 0 || depth >= MaxDepth)
            {
                ...
                return new Node(id, $"Class: {id}");
            }

            var column = FindAttributeWithLargestInfoGain(vectors,numberOfClasses, entropy);
        ...
        {
            return this.GraphToVisualize.Vertices.
                Where(node => GraphToVisualize.InDegree(node) == 0).First();
        }
        ...
        private int FindAttributeWithLargestInfoGain(IEnumerable<double[]> vectors, int numberOfClasses, double entropy)
        {
